# -*- coding: utf-8 -*-
"""
Train + evaluate Attention U-Net [09] (Oktay et al., 2018) on the cleaned/grouped
BUSI segmentation split, then write a metrics JSON in the shared results format.

Reuses the shared pipeline in common/ (identical preprocessing/split/metrics as the
other segmentation baselines, so results are directly comparable):
  * data.make_split()            -> leakage-free train/val/test (benign+malignant)
  * data.SegDataset(rgb3=False)  -> 1-channel [-1,1] images + binary masks (from scratch)
  * metrics.aggregate_seg / detection_metrics -> Dice/IoU/pixel + Yap detection scores
  * runner.*                     -> seeding, DEVICE, results JSON, curve, example grid

Loss is Dice+BCE (same as U-Net [03]); selection is by best validation Dice.
Run:  python Models/Attention-U-Net_Oktay2018/train.py
Outputs: Models/results/AttentionUNet_Oktay2018.json (+ figures, checkpoint).
"""
import os, sys, time
import numpy as np
import torch
...
def dice_bce_loss(logits, target):
    """Dice + BCE (many tiny lesions -> pixel imbalance; same loss as U-Net [03])."""
...
def main():
    ...
    model = AttentionUNet(1, 1, base=64).to(R.DEVICE)
    ...
    results = {
        "model": "Attention U-Net", "paper": "Oktay et al. (2018), arXiv:1804.03999 [09]",
        "task": "lesion segmentation (benign+malignant)",
        "our_work": "from-scratch PyTorch U-Net + additive attention gates on every skip "
                    "connection (Oktay Eq. 1-2); Dice+BCE loss; letterbox preprocessing; "
                    "grouped leakage-free split shared with the other segmentation baselines",
